...
class CombateBot
{
    ...
    function get_habilidade($lvl, $mp_max, $alvo)
    {
        $tipo = rand(1, 6) <= 5 ? TIPO_SKILL_ATAQUE_CLASSE : TIPO_SKILL_BUFF_CLASSE;
        if ($tipo == TIPO_SKILL_ATAQUE_CLASSE) {
            $maior_consumo = 0;
            $habilidades = MapLoader::filter("skil_atk",
                function ($habilidade) use ($mp_max, $lvl, &$maior_consumo) {
                    if ($habilidade["consumo"] < $mp_max
                        && $habilidade["requisito_lvl"] <= $lvl
                        && $habilidade["requisito_classe"] != 0) {
                        if ($habilidade["consumo"] > $maior_consumo) {
                            $maior_consumo = $habilidade["consumo"];
                        }
                        return true;
                    }
                    return false;
            ...
            $habilidades_possiveis = array_filter($habilidades, function ($habilidade) use ($maior_consumo) {
                return $habilidade["consumo"] == $maior_consumo;
            });

            if (! count($habilidades_possiveis)) {
                $habilidades_possiveis = array(
                    MapLoader::load("skil_atk")[0] // soco
                );
            }
        ...
        if (! isset($habilidade["img"])) {
            $habilidade["img"] = isset($habilidade["icone_padrao"])
                ? $habilidade["icone_padrao"]
                : rand(1, SKILLS_ICONS_MAX);
        }
