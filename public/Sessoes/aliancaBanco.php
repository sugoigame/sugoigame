<div class="panel-heading">
    Banco da
    <?= ($usuario["faccao"] == 0) ? "Frota" : "Aliança" ?>
</div>
...
<div class="panel-body">
        ...
        <ul class="list-group">
            <?php while ($log = $result->fetch_array()) : ?>
                <li class="list-group-item">
                    <h4>
                        <?= $log["usuario"] ?>
                        <?= ($log["tipo"] == 1) ? "depositou" : "saqueou"; ?>
                        <?= $log["item"] ?>
                    </h4>
                    <p>
                        <?= date("d/m/Y - H:i", strtotime($log["momento"])) ?>
                    </p>
                </li>
            <?php endwhile; ?>
        </ul>
    ...
    Dinheiro no banco da
    <?= ($usuario["faccao"] == 0) ? "Frota" : "Aliança" ?>:
    <img src="Imagens/Icones/Berries.png" alt="berries" />
    <?= mascara_berries($userDetails->ally["banco"]) ?><br /><br />
    <!--
                    ...
                    <div class="form-inline">
                        <div class="form-group">
                            <?php if ($item["quant"] > 1) : ?>
                                ...
                                <input class="form-control" type="number" id="saque_<?= $x ?>_input" size="5" type="number" min="1"
                                    max="<?= $item["quant"] ?>" />
                            <?php endif; ?>
                            <button id="saque_<?= $x ?>" data="item=<?= $item["cod_item"] ?>&tipo=<?= $item["tipo_item"] ?>"
