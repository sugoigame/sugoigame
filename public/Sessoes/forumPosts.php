...
<div class="panel-body">
    ...
    <div class="text-right">
        <p>
            ...
            <button class="btn btn-info link_content"
                href="./?ses=forumTopics&categoria=<?= $topico["categoria_id"] ?>">
    ...
    <?php $posts = $connection->run(
        "SELECT p.*, u.*, pers.nome, pers.img, pers.skin_r, pers.skin_c, pers.borda, pers.fama_ameaca,
                    ...
                    <div class="col-md-9 text-left">
                        <div class="pull-right">
                            <?php if ($topico["criador_id"] == $userDetails->tripulacao["id"]
                                ...
                                <?php if ($post["oculto"]) : ?>
                                    <button class="btn btn-info link_send" href="link_Forum/ocultar.php?post=<?= $post["id"] ?>">
                            ...
                            <?php if ($post["tripulacao_id"] != $userDetails->tripulacao["id"]) : ?>
                                <button class="btn btn-success link_send" data-toggle="tooltip"
                                    ...
                                    title="<?= $post["likes"] ?> curtiram" <?= $post["i_like"] == 1 ? "disabled" : "" ?>>
                                ...
                                <button class="btn btn-danger link_send" data-toggle="tooltip"
                                    ...
                                    title="<?= $post["deslikes"] ?> não curtiram" <?= $post["i_like"] == 2 ? "disabled" : "" ?>>
                                    <i class="fa fa-thumbs-down"></i> <?= $post["deslikes"] ?>
                                </button>
                            <?php else : ?>
                                <span class="text-success" data-toggle="tooltip" title="<?= $post["likes"] ?> curtiram">
                                    <i class="fa fa-thumbs-up"></i> <?= $post["likes"] ?>
                                </span>
                                <span class="text-danger" data-toggle="tooltip" title="<?= $post["deslikes"] ?> não curtiram">
                                    <i class="fa fa-thumbs-down"></i> <?= $post["deslikes"] ?>
                                </span>
                            <?php endif; ?>
                        </div>
