...
<div class="panel-body">
    ...
    <?php if ($userDetails->capitao["lvl"] >= 15) : ?>
        <h5>Escolha um nome e uma forma de pagamento para criar sua própria
            <?= $termo_correto ?>:
        </h5>
        ...
        <form method="post" action="Scripts/Alianca/alianca_criar.php">
            ...
            <button class="btn btn-success" type="submit">Criar minha
                <?= $termo_correto ?>
            </button>
        </form>
    <?php else : ?>
        É necessário ter um capitão no nível 15 ou superior para criar uma
        <?= $termo_correto ?>
    <?php endif; ?>
    <h4>Convites</h4>
            ...
            <?php while ($convite = $result->fetch_array()) : ?>
                <li class="list-group-item">
                    <h4>
                        ...
                        te convidou para participar de uma
                        <?= $termo_correto ?>
                    </h4>
                    <p>
    ...
    <?php else : ?>
        <p>Você não recebeu nenhum convite para participar de nenhuma
            <?= $termo_correto ?> ainda
        </p>
    <?php endif; ?>
</div>
