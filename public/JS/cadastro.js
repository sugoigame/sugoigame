$(function () {
    ...
    $(".capitao-selectable-img").click(function () {
        ...
        $("#img_capitao").attr(
            "src",
            "Imagens/Personagens/Big/" + img + "(0).jpg"
        );
...
function verifica_capitao() {
    ...
    if (document.getElementById("capitao").value.length < 3) {
        ...
        document.getElementById("capitao_status_fail").innerHTML =
            "O nome do capitão deve conter no minimo 3 caracteres";
    } else {
        $.ajax({
            type: "get",
            data: "capitao=" + input,
            ...
            success: function (retorno) {
                if (retorno == 1) {
                    //ok
                    document.getElementById("capitao_status").src =
                        "Imagens/Icones/1.gif";
                    document.getElementById("capitao_status_fail").innerHTML =
                        "";
                } else {
                    //fail
                    document.getElementById("capitao_status").src =
                        "Imagens/Icones/0.gif";
                    document.getElementById("capitao_status_fail").innerHTML =
                        "O nome do capitão informado já está cadastrado";
                }
            },
        });
    }
    ...
    if (document.getElementById("login").value.length < 5) {
        ...
        $.ajax({
            ...
            success: function (retorno) {
                if (retorno == 1) {
                    //ok
                    document.getElementById("login_status").src =
                        "Imagens/Icones/1.gif";
                    document.getElementById("login_status_fail").innerHTML = "";
                } else {
                    //fail
...
function verifica_apelido() {
    ...
    if (document.getElementById("apelido").value.length < 3) {
        ...
        document.getElementById("apelido_status_fail").innerHTML =
            "O apelido da tripulação deve conter no minimo 3 caracteres";
    } else {
        $.ajax({
            type: "get",
            data: "apelido=" + input,
            ...
            success: function (retorno) {
                if (retorno == 1) {
                    //ok
                    document.getElementById("apelido_status").src =
                        "Imagens/Icones/1.gif";
                    document.getElementById("apelido_status_fail").innerHTML =
                        "";
                } else {
                    ...
                    document.getElementById("apelido_status").src =
                        "Imagens/Icones/0.gif";
                    document.getElementById("apelido_status_fail").innerHTML =
                        "O apelido informado já está cadastrado";
                }
            },
        });
    }
...
function verifica_nome() {
    ...
    if (document.getElementById("nome").value.length < 5) {
        ...
        document.getElementById("nome_status_fail").innerHTML =
            "O nome deve conter no minimo 5 caracteres";
    } else {
    ...
    if (!ermai.test(mai) || mai.length == 0) {
        ...
        $.ajax({
            ...
            success: function (retorno) {
                if (retorno == 1) {
                    //ok
                    document.getElementById("email_status").src =
                        "Imagens/Icones/1.gif";
                    document.getElementById("email_status_fail").innerHTML = "";
                } else {
                    //fail
...
function verifica_cadastro(event) {
    ...
    if (document.getElementById("apelido").value.length < 3) {
        mensagem +=
            "<br>* O apelido da tripulação deve conter no minimo 3 caracteres";
        erro = true;
    }

    if (document.getElementById("capitao").value.length < 3) {
        mensagem +=
            "<br>* O nome do capitão deve conter no minimo 3 caracteres";
        erro = true;
    }
