...
if (FALSE) {
	...
	for ($x = 0; $x < sizeof($personagem); $x++) {
		...
		$connection->run($query) or die("nao foi possivel atualizar o personagem " . $personagem[$x]["cod"]);
	}

	$fuga = $usuario["fugas"] + 1;
	$query = "UPDATE tb_usuarios SET fugas='$fuga' WHERE id='" . $usuario["id"] . "'";
	$connection->run($query) or die("nao foi possivel contabilizar a fuga");

	$query = "DELETE FROM tb_combate_npc WHERE id='" . $usuario["id"] . "'";
	$connection->run($query) or die("nao foi possivel remover combate");
	...
	for ($x = 0; $sql = $result->fetch_array(); $x++) {
		...
		if ($espera > 0) {
			...
			$connection->run($query2) or die("nao foi possivel remover espera da skil " . $sql["cod_skil"]);
		} else {
	...
	while ($buff = $result->fetch_array()) {
		if ($buff["espera"] > 0) {
			...
			$query2 = "UPDATE tb_combate_buff SET espera='$espera' 
				...
				LIMIT 1";
			$connection->run($query2) or die("nao foi possivel reduzir duração do buff");
		} else {
			$query2 = "DELETE FROM tb_combate_buff 
				WHERE id='" . $usuario["id"] . "' AND cod='" . $buff["cod"] . "' AND atr='" . $buff["atr"] . "' 
				AND efeito='" . $buff["efeito"] . "' AND cod_buff='" . $buff["cod_buff"] . "' AND espera='" . $buff["espera"] . "'
				LIMIT 1";
			$connection->run($query2) or die("nao foi possivel remover o buff");
		}
	}
	...
	$query = "SELECT * FROM tb_combate_personagens WHERE id='" . $usuario["id"] . "'";
	$result = $connection->run($query);
	while ($personagem_em_combate = $result->fetch_array()) {
		$nmp = $personagem_em_combate["mp"] + ($personagem_em_combate["mp"] * 0.01);
		if ($nmp > $personagem_em_combate["mp_max"])
			$nmp = $personagem_em_combate["mp_max"];
		$query2 = "UPDATE tb_combate_personagens SET mp='$nmp' WHERE cod='" . $personagem_em_combate["cod"] . "'";
		$connection->run($query2) or die("Nao foi possivel atualizar buffs");
	}
...
if (isset($usuario["npc"]["img_npc"])) {
	...
	$result = $connection->run($query);
	$personagem_info = $result->fetch_array();
	...
	if ((rand(1 * 10, 100 * 10) / 10) <= $esquiva) {
		...
		if ($tipo_skil_npc == 1) {
			...
			$connection->run($query) or die("Nao foi possivel atualizar energia do npc");

			$query = "UPDATE tb_combate_personagens SET hp='$nhp' WHERE id='" . $usuario["id"] . "' AND cod='" . $personagem_alvo[$cod_rand]["cod"] . "'";
			$connection->run($query) or die("Nao foi possivel atualizar hp do alvo do npc");
			...
			$relatorio .= "_";
			$query = "UPDATE tb_combate_npc SET relatorio='$relatorio' WHERE id='" . $usuario["id"] . "'";
			$connection->run($query) or die("nao foi possivel relatar turno npc");

			$move = 5;
			$query = "UPDATE tb_combate_npc SET move='$move' WHERE id='" . $usuario["id"] . "'";
			$connection->run($query) or die("nao doi possivel registrar movimento");
	...
	$connection->run($query) or die("nao foi possivel relatar fim de turno");
	echo "Você não conseguiu fugir";
}
