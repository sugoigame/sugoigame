...
if (!isset($_POST["code"]) OR !preg_match(INT_FORMAT, $_POST["code"])) {
    mysql_close();
    header("location:../../?ses=vipComprar&msg=Você informou algum dado inválido.");
    exit();
}
...
if ($error) {
    mysql_close();
    header("location:../../?ses=vipComprar&msg=Ocorreu um erro ao tentar se conectar com a API de pagamentos.");
    exit();
}
