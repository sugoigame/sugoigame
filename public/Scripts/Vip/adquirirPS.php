...
if ($result->count() < 1) {
    ...
    $paymentRequest->addItem($plano['id'], $plano['nome'], 1, $plano['valor']);
    ...
    $connection->run("INSERT INTO tb_vip_compras (conta_id,plano_id,gateway) VALUE (?,?,?)", "iis", [$userDetails->conta['conta_id'], $plano['id'], 'PagSeguro']);
