...
{ ?>
    <a href="#"
        class="noHref slot slot_<?= $slot ?> <?= isset($equips_info[$slot]) ? "equipamentos_classe_" . ($equips_info[$slot]["categoria"]) : "" ?>"
        data-content="-" data-toggle="popover" data-placement="bottom" data-html="true" data-trigger="focus" data-template='
        ...
        <div class="row">
            ...
            <div class="col-md-6 personagem-equipamentos">
                <ul class="text-left personagem-atributos">
                    <?php for ($i = 1; $i < 8; $i++) : ?>
                        <li>
                            <img src="Imagens/Icones/<?= nome_atributo_img($i) ?>.png" width="25px">
                            +
            ...
            <div class="col-md-3">
                ...
                <a href="#" data-trigger="focus"
                    class="noHref slot slot_acessorio <?= isset($acessorio["categoria"]) ? "equipamentos_classe_" . $acessorio["categoria"] : "" ?>"
                    data-content="-" data-toggle="popover" data-placement="bottom" data-html="true" data-template='
                                     <div class="equip-info">
                                        <div class="panel panel-default">
                ...
                <div class="col-md-3 list-group-item">
                    ...
                    <div>
                        <?php if ($item["tipo_item"] == TIPO_ITEM_ACESSORIO) : ?>
                            <button
                                href="link_Personagem/equipar.php?item=<?= $item["cod_acessorio"] ?>&person=<?= $pers["cod"] ?>"
                        ...
                        <?php else : ?>
                            <?php if ($pers["lvl"] >= $item["lvl"] and ($item["requisito"] == $pers["classe"] or $item["requisito"] == 0)) : ?>
                                <?php if ($item["slot"] <= 8) : ?>
                                    <button
                                        href="link_Personagem/equipamento_equipar.php?item=<?= $item["cod_equipamento"] ?>&pers=<?= $pers["cod"] ?>&slot=<?= $item["slot"] ?>"
                                        class="link_send btn btn-success">
                                        Equipar
                                    </button>
                                <?php elseif ($item["slot"] == 9) : ?>
                                    <button
                                        href="link_Personagem/equipamento_equipar.php?item=<?= $item["cod_equipamento"] ?>&pers=<?= $pers["cod"] ?>&slot=7"
                                        class="link_send btn btn-success">
                                        Primeira mão
                                    </button><br />
                                    <button
                                        href="link_Personagem/equipamento_equipar.php?item=<?= $item["cod_equipamento"] ?>&pers=<?= $pers["cod"] ?>&slot=8"
                                        class="link_send btn btn-success">
                                        Segunda mão
                                    </button><br />
                                    <button
                                        href="link_Personagem/equipamento_equipar.php?item=<?= $item["cod_equipamento"] ?>&pers=<?= $pers["cod"] ?>&slot=1"
                                        class="link_send btn btn-success">
                                        Cabeça
                                    </button>
                                <?php else : ?>
