...
class HabilidadeBotaoCustomizar
{
    public static function render($pers, $habilidade, $animacoes)
    {
        ?>
        <?php $modal_id = $pers["cod"] . "_" . $habilidade["cod"]; ?>
        ...
        <button id="customiza-<?= $modal_id ?>" class="btn btn-sm btn-info">Customizar</button>

        <div class="modal fade" id="modal-edit-skill-<?= $modal_id ?>">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <?php if (is_editavel($habilidade)) : ?>
                                <div class="col-md-6">
                                    <form action="Vip/customiza_skill" class="ajax_form"
                                        onsubmit="$('#modal-edit-skill-<?= $modal_id ?>').modal('hide');"
                                        data-question="Deseja customizar a habilidade?" id="form-custumza-skill" method="POST">
                                        <input value="<?= $pers["cod"] ?>" name="codpers" type="hidden">
                                        <input value="<?= $habilidade["cod"] ?>" name="codskil" type="hidden">
                                        <input id="skill-input-<?= $modal_id ?>" name="img" type="hidden"
                                            value="<?= $habilidade["icone"] ?>" required>

                                        <label>Selecione uma imagem:</label>
                                        <img width="40px" id="skill-img-<?= $modal_id ?>"
                                        ...
                                        <div class="selecao_img" style="display: none" id="skill-list-<?= $modal_id ?>">
                                        </div>
                                        <div class="form-group">
                                            <label>Nome da habilidade</label>
                                            <input name="nome" size="10" maxlength="20" class="form-control"
                                                value="<?= $habilidade["nome"] ?>" required>
                                        </div>

                                        <div class="form-group">
                                            <label>Descrição da habilidade</label>
                                            <textarea cols="18" maxlength="300" name="descricao" class="form-control"
                                ...
                                <div class="col-md-6">
                                    ...
                                    <p>
                                        <button data-effect="<?= $habilidade["animacao"] ?>" class="play-effect btn btn-primary">
                                            <?= $habilidade["animacao"] ?> <i class="fa fa-play"></i>
                                        </button>
                                    </p>
                                    <h4>Animações disponíveis:</h4>
                                    <?php if (count($animacoes)) : ?>
                                        <form class="ajax_form" method="post" action="Personagem/mudar_animacao_skill"
                                            onsubmit="$('#modal-edit-skill-<?= $modal_id ?>').modal('hide');"
                                            ...
                                            <input type="hidden" name="cod_skil" value="<?= $habilidade["cod_skil"] ?>" />
                                            <div class="form-group">
