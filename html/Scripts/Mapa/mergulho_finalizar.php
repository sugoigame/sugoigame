...
if (!$personagem || $personagem["profissao"] != PROFISSAO_MERGULHADOR) {
    $protector->exit_error("Personagem inválido");
}

$mergulhadores = $userDetails->mergulhadores;
...
$tempo = 3600 - (($userDetails->lvl_mergulhador - 1) * 180);
$preco = ceil(($userDetails->tripulacao["mergulho"] - atual_segundo()) / $tempo) * 2 - 1;
...
for ($x = 1; $x <= $quant; $x++) {
    ...
    if ($rand <= 24) {
        ...
        $berries = round(($userDetails->lvl_mergulhador * 300000) * (rand(50, 100) / 100));
...
$connection->run("UPDATE tb_personagens SET profissao_xp = LEAST(profissao_xp_max, profissao_xp + 5) WHERE profissao = ? AND id = ? AND ativo = 1",
    "ii", array(PROFISSAO_MERGULHADOR, $userDetails->tripulacao["id"]));
...
if ($reducao = $userDetails->buffs->get_efeito("reducao_tempo_mergulho")) {
    $tempo -= $reducao;
}

if ($userDetails->tripulacao["mergulho"] > atual_segundo()) {
    $tempo += $userDetails->tripulacao["mergulho"];
} else {
    $tempo += atual_segundo();
}

$connection->run("UPDATE tb_usuarios SET mergulho = ? WHERE id = ?",
