...
<div class="panel-body">
    ...
    <ul class="text-left">
        <?php $total = $connection->run("SELECT count(id) AS total FROM tb_personagens WHERE haki_lvl = 50 GROUP BY id HAVING total >= 15")->count(); ?>
        <li>Tripulações full Haki: <?= $total ?></li>

        <?php $total = $connection->run("SELECT count(cod) AS total FROM tb_personagens WHERE haki_lvl = 50")->fetch_array()["total"]; ?>
        <li>Personagens full Haki: <?= $total ?></li>

        <?php $total = $connection->run("SELECT count(DISTINCT id) AS total FROM tb_personagens WHERE haki_lvl = 50")->fetch_array()["total"]; ?>
        ...
        <li>Personagens com 25 de Haki ou mais: <?= $total ?></li>

        <?php $total = $connection->run("SELECT count(DISTINCT id) AS total FROM tb_personagens WHERE haki_lvl >= 25")->fetch_array()["total"]; ?>
        <li>Tripulações com pelo menos 1 personagem com 25 de Haki ou mais: <?= $total ?></li>
