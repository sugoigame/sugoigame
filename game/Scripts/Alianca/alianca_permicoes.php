<?php
	...
	if(!$inally){
		mysql_close();
		header("location:../../index.php?msg=Voce nao faz parte de uma aliança");
		exit();
	}
	...
	if($permicao["autoridade"]!=0){
		mysql_close();
		header("location:../../index.php?msg=Você não tem permissão para alterar cargos.");
		exit();
	}
	...
	OR !preg_match("/^[\d]+$/", $nperm4)){
		mysql_close();
		header("location:../../index.php?msg=Voce informou algum caracter invalido");
		exit();
	}
	...
	mysql_close();
	header("location:../../index.php?ses=alianca&msg2=Permissões alteradas");
?>