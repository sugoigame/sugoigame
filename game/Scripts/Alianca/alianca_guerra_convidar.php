<?php
	...
	if(!$conect){
		mysql_close();
		header("location:../../index.php?msg=Você precisa estar logado.");
		exit();
	}
	if(!$inally){
		mysql_close();
		header("location:../../index.php?msg=Você não faz parte de uma aliança");
		exit();
	}
	
	if($usuario["alianca"]["lvl"]<5){
		mysql_close();
		header("location:../../index.php?msg=É necessário ter nível 5 para entrar em guerra!");
		exit();
	}
	...
	if(mysql_num_rows($result)<15){
		mysql_close();
		header("location:../../index.php?msg=Sua Aliança precisa de 15 membros para participar de uma guerra.");
		exit();
	}
	...
	if(substr($usuario["alianca"][$permicao["autoridade"]],4,1)==0){
		mysql_close();
		header("location:../../index.php?msg=Você não tem permissão para desafiar jogadores.");
		exit();
	}
	
	if(!isset($_POST["nome"]) OR !isset($_POST["tipo"])){
		mysql_close();
		header("location:../../index.php?msg=Você informou algum caracter inválido.");
		exit();
	}
	...
	if(mysql_num_rows($result)==0){
		mysql_close();
		header("location:../../index.php?msg=Aliança não encontrada.");
		exit();
	}
	...
	if($id_conv["lvl"]<5){
		mysql_close();
		header("location:../../index.php?msg=É necessário que o oponente esteje no nível 5 para entrar em guerra!");
		exit();
	}
	
	if($id_conv["cod_alianca"]==$usuario["alianca"]["cod_alianca"]){
		mysql_close();
		header("location:../../index.php?msg=Você não pode entrar em guerra com você.");
		exit();
	}
	...
	if(mysql_num_rows($result)<15){
		mysql_close();
		header("location:../../index.php?msg=A aliança oponente precisa de 15 membros para participar de uma guerra.");
		exit();
	}
	...
	if($lider["faccao"]==0 AND $usuario["faccao"]==0){
		mysql_close();
		header("location:../../index.php?msg=Marinheiros não podem entrar em guerra com marinheiros.");
		exit();
	}
	...
	if(mysql_num_rows($result)!=0){
		mysql_close();
		header("location:../../index.php?msg=Essa aliança já tem um pedido de guerra pendente.");
		exit();
	}
	...
	mysql_close();
	header("location:../../index.php?ses=aliancaDiplomacia");
?>