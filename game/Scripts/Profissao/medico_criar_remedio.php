...
if ($inmissao) {
    mysql_close();
    header("location:../../index.php?msg=Você está ocupado em uma missão neste meomento.");
    exit;
}
if (!isset($_GET["pers"])) {
    mysql_close();
...
if (!preg_match("/^[\d]+$/", $pers)) {
    mysql_close();
    header("location:../../index.php?msg=Você informou algum caracter inválido.1");
    exit();
}
if (!preg_match("/^[\d]+$/", $item)) {
    mysql_close();
    header("location:../../index.php?msg=Você informou algum caracter inválido.2");
    exit();
}
if (!preg_match("/^[\d]+$/", $quant_faz)) {
    mysql_close();
    header("location:../../index.php?msg=Você informou algum caracter inválido.3");
    exit();
}

if ($quant_faz < 0) {
    mysql_close();
    header("location:../../index.php?msg=Você informou algum caracter inválido.4");
    exit();
}
...
if (mysql_num_rows($result) >= $usuario["capacidade_iventario"]) {
    mysql_close();
    header("location:../../index.php?msg=Seu iventário está lotado.");
    exit;
}
...
if ($personagem["profissao"] != 3) {
    mysql_close();
    header("location:../../index.php?msg=Este personagem não é um médico.");
    exit;
}
...
if ($usuario["berries"] < $item["preco"]) {
    mysql_close();
    header("location:../../index.php?msg=Você não tem dinheiro para essa quantidade de itens.");
    exit;
}

if ($personagem["profissao_lvl"] < $item["requisito_lvl"]) {
    mysql_close();
    header("location:../../index.php?msg=Você não cumpre os requisitos para fazer este item.");
    exit;
}
...
mysql_close();
header("location:../../index.php?ses=profissoes");
?>