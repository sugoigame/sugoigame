...
if (!$conect) {
    mysql_close();
    header("location:../../index.php?msg=Você precisa estar logado para executar essa ação.");
    exit;
}
if ($inmissao) {
    mysql_close();
...
if ($personagem["profissao"] != 7) {
    mysql_close();
    header("location../../index.php?msg=Este personagem não é um cozinheiro.");
    exit;
}
...
if ($usuario["berries"] < $item["preco"]) {
    mysql_close();
    header("location../../index.php?msg=Você não tem dinheiro para fazer essa quantidade de comida.");
    exit;
}
