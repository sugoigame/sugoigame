...
if (!isset($_POST["nome"])
    ...
    header("location:../../index.php?ses=cadastro&msg=Formulário incompleto.");
    exit();
}
...
if (isset($_POST["padrinho"])) {
    ...
    if (!validate_alphanumeric($padrinho)) {
        header("location:../../index.php?ses=cadastro&msg=Você informou algum caracter inválido.");
        exit();
    }
}

if (strlen($nome) < 5 || strlen($senha) < 5) {
    header("location:../../index.php?ses=cadastro&msg=Algum valor informado nao atende os requisitos de tamanho.");
    exit();
}
...
if ($result->count()) {
    header("location:../../index.php?ses=cadastro&msg=O email informado já está cadastrado.");
    exit();
}
