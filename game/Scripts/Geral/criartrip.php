...
if (!$contaOk) {
    mysql_close();
    header("location:../../index.php?msg=Vocce precisa estar logado.");
    exit();
}
...
if (!isset($_POST["faccao"])
    ...
    mysql_close();
    header("location:../../index.php?ses=seltrip&msg=Formulário incompleto.");
    exit();
}
...
if (!preg_match("/^[\d]+$/", $faccao)) {
    mysql_close();
    header("location:../../index.php?ses=seltrip&msg=Você informou algum caracter inválido4.");
    exit();
}
if (!preg_match("/^[\w ]+$/", $capitao)) {
    mysql_close();
    header("location:../../index.php?ses=seltrip&msg=Você informou algum caracter inválido5.");
    exit();
}
if (!preg_match("/^[\d]+$/", $icon)) {
    mysql_close();
    header("location:../../index.php?ses=seltrip&msg=Você informou algum caracter inválido6.");
    exit();
}
if (!preg_match("/^[\d]+$/", $oceano)) {
    mysql_close();
    header("location:../../index.php?ses=seltrip&msg=Você informou algum caracter inválido7.");
    exit();
}
if (!preg_match("/^[\w ]+$/", $apelido)) {
    mysql_close();
    header("location:../../index.php?ses=seltrip&msg=Você informou algum caracter inválido8.");
    exit();
}
...
if ($erro) {
    mysql_close();
    header("location:../../index.php?ses=seltrip&msg=Algum valor informado nao atende os requisitos de tamanho.");
    exit();
} else {
    ...
    if ($cont != 0) {
        mysql_close();
        header("location:../../index.php?ses=seltrip&msg=O nome de capitão informado já está cadastrado.");
        exit();
    } else {
        ...
        if (mysql_num_rows($result) >= 3) {
            mysql_close();
            header("location:../../index.php?ses=seltrip&msg=O limite de tripulações por conta é de 3.");
            exit();
        }
        ...
        $ssga->set_event('Tripulacao', 'criar', 'nova_tripulacao', 1);
        $ssga->send();
